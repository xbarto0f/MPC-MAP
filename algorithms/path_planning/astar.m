...
%%
% Compute penalty map manually (approximate distance to obstacles)
penalty_map = compute_penalty_map(occupancy_grid);
penalty_factor = 5;
...
while ~isempty(open_list)
    ...
    for d = 1:size(directions, 1)
        ...
        if is_valid(neighbor, occupancy_grid, closed_list)
            new_g = g + norm(directions(d, :));
            h = heuristic(neighbor, goal);
            penalty = penalty_map(neighbor(2), neighbor(1));  % row, col
            f = new_g + h + penalty_factor * penalty;
...
function valid = is_valid(node, occupancy_grid, closed_list)
    ...
    valid = x > 0 && y > 0 && ...
            x <= size(occupancy_grid, 2) && ...
            y <= size(occupancy_grid, 1) && ...
            occupancy_grid(y, x) == 0 && ...
            ~isKey(closed_list, key);
end
...
function penalty_map = compute_penalty_map(occupancy_grid)
    [rows, cols] = size(occupancy_grid);
    penalty_map = inf(rows, cols);
    visited = false(rows, cols);
    queue = [];

    % Initialize queue with obstacle positions
    for y = 1:rows
        for x = 1:cols
